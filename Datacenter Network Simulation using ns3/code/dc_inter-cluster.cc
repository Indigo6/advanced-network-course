...
int main(int argc, char *argv[])
{
    ...
    for(i=0; i<2; i++){
        c2aNodes[i].Add(Nodes.Get(0));//c2aNode[0]:
    ...
    CsmaHelper csma[2];
    for(i=0; i<2; i++){
        csma[i].SetChannelAttribute("DataRate", StringValue("1Mbps"));
    ...
    for (int i=0; i<4; i++) {
        ...
        sinkApp[i] = packetSinkHelper.Install (Nodes.Get(end[i]+6));
    ...
    for (int i=0; i<4; i++) {
        ...
        clientApp[i] = client.Install (Nodes.Get(begin[i]+6));
    ...
    /***********Enable pcap**************/
    pointToPoint.EnablePcapAll ("Pattern1_p2p");
    // csma[0].EnablePcapAll ("Pattern1_csma1");
    csma[1].EnablePcapAll ("Pattern1_csma2");
